...
public class MainContentMusicHolder extends RecyclerView.ViewHolder implements BaseViewHolder<StreamItemCluster> {
    ...
    private MainContentMusicHolder(Fragment fragment, View itemView) {
        super(itemView);
        ...
        title.setText(this.mFragment.getActivity().getResources().getString(R.string.category_music_text));
        title.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.ic_music, 0, 0, 0);
        title.setCompoundDrawablePadding(5);

        createFlexWeightLayoutParams();
    }

    public static MainContentMusicHolder newInstance(ViewGroup parent, Fragment fragment) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_recommend_layout,
                parent, false);
        return new MainContentMusicHolder(fragment, view);
    }
