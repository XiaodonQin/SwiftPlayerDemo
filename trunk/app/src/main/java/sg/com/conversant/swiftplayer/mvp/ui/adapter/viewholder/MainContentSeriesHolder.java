...
public class MainContentSeriesHolder extends RecyclerView.ViewHolder implements BaseViewHolder<StreamItemCluster> {
    ...
    private MainContentSeriesHolder(Fragment fragment, View itemView) {
        super(itemView);
        ...
        title.setText(this.mFragment.getActivity().getResources().getString(R.string.category_series_text));
        title.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.ic_series, 0, 0, 0);
        title.setCompoundDrawablePadding(5);

        createFlexWeightLayoutParams();
    }

    public static MainContentSeriesHolder newInstance(ViewGroup parent, Fragment fragment) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_recommend_layout,
                parent, false);
        return new MainContentSeriesHolder(fragment, view);
    }
    ...
    private class OnClickFlexItemListener implements View.OnClickListener {
        @Override
    ...
    private void createFlexWeightLayoutParams() {
        ...
        int coverWidth = (outSize.x - 6 * margin) / 2;// 每行三个
