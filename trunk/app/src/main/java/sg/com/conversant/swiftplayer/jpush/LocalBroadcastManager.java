package sg.com.conversant.swiftplayer.jpush;
...
public final class LocalBroadcastManager {
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                L.v("LocalBroadcastManager" + " Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            }

            ArrayList entries = (ArrayList) this.mActions.get(intent.getAction());
            if (entries != null) {
                if (debug) {
                    L.v("LocalBroadcastManager" + " Action list: " + entries);
                }
                ...
                for (i = 0; i < entries.size(); ++i) {
                    ...
                    if (debug) {
                        L.v("LocalBroadcastManager" + " Matching against filter " + receiver.filter);
                    }

                    if (receiver.broadcasting) {
                        if (debug) {
                            L.v("LocalBroadcastManager" + " Filter\'s target already added");
                        }
                    } else {
                        int match = receiver.filter.match(action, type, scheme, data, categories, "LocalBroadcastManager");
                        if (match >= 0) {
                            if (debug) {
                                L.v("LocalBroadcastManager" + " Filter matched!  match=0x" + Integer.toHexString(match));
                            }
                            ...
                            L.v("LocalBroadcastManager" + " Filter did not match: " + reason);
                        }
                    }
                }
