...
import sg.com.conversant.swiftplayer.mvp.ui.activity.StreamDetailActivity;
...
public class MainContentRecommendHolder extends RecyclerView.ViewHolder implements BaseViewHolder<StreamItemCluster> {
    ...
    private MainContentRecommendHolder(Fragment fragment, View itemView) {
        super(itemView);
        ...
        title.setText(this.mFragment.getActivity().getResources().getString(R.string.item_recommend_text));
        title.setCompoundDrawablesWithIntrinsicBounds(R.mipmap.ic_hot, 0, 0, 0);
        title.setCompoundDrawablePadding(5);

        createFlexWeightLayoutParams();
    }

    public static MainContentRecommendHolder newInstance(ViewGroup parent, Fragment fragment) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_recommend_layout,
                parent, false);
        return new MainContentRecommendHolder(fragment, view);
    }
    ...
    private void initFlexLayout(List<StreamItem> items) {
        ...
        for (int i = 0; i < iDataSize; i++) {
            ...
            flexItemHolder.cardView.setTag(streamItem);
            flexItemHolder.cardView.setOnClickListener(new OnClickFlexItemListener());
        }
    }
    ...
    public class OnClickFlexItemListener implements View.OnClickListener {
        @Override
        public void onClick(View v) {
            StreamItem item = (StreamItem) v.getTag();
            Intent mIntent = new Intent(mActivity, StreamDetailActivity.class);
            mIntent.putExtra(API.BUNDLE_PARAMS.STREAM_ITEM, item);
